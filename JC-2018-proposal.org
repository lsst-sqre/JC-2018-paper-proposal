...
The Large Synoptic Survey Telescope is a very ambitious project, funded
by NSF and DOE, to produce an extremely large survey catalogue of the
southern sky.  The scale of the data that will be collected implies that
traditional methods of astronomical computing will be inadequate for its
analysis.  We propose a new paradigm, in which efficient exploration of
the problem space--required for significant data analysis--is
facilitated by the combination of JupyterLab, JupyterHub, and
Kubernetes.  We discuss challenges (authentication, authorization, data
access, et al.)  arising from this approach and the specific solutions
LSST Data Management has found to them.
...
*** The infrastructure is standard and modular.  You drop in your own science stack and go.
...
*** Problem 8: startup time and user frustrationr containers are not normal containers.  To wit: they're on the order
